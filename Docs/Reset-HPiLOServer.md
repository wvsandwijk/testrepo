...
```
Reset-HPiLOServer [-OutputType <String>] [-Username <Object>] [-Password <Object>] [-Credential <Object>]
 [-Force] [-Server <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Reset-HPiLOServer cmdlet invokes a power cycle (off and then back on) on the host server on which the iLO is operating.
...
### EXAMPLE 2
```
PS C:\> Reset-HPiLOServer -Server $Server
...
$Server is passed as parameter to Reset-HPiLOServer.
...
### EXAMPLE 3
```
PS C:\> Reset-HPiLOServer -Server $Server -Username $Username -Password $Password
```
...
Without this parameter, Reset-HPiLOServer requires you to provide the values of all required parameters.
