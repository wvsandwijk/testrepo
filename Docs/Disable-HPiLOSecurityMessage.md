...
```
Disable-HPiLOSecurityMessage [-OutputType <String>] [-Username <Object>] [-Password <Object>]
 [-Credential <Object>] [-Force] [-Server <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Disable-HPiLOSecurityMessage cmdlet disables the display of security text message in the iLO Login Banner displayed on the iLO login screen.
You must have configure iLO Setting privileges to make changes to the banner.
You must have Config ILO privilege to execute the command.
...
### EXAMPLE 1
```
PS C:\> Disable-HPiLOSecurityMessage
...
### EXAMPLE 2
```
PS C:\> Disable-HPiLOSecurityMessage -Server $Server
...
$Server is passed as parameter to Disable-HPiLOSecurityMessage.
...
### EXAMPLE 3
```
PS C:\> Disable-HPiLOSecurityMessage -Server $Server -Username $Username -Password $Password
```

This command shows that the list of iLO server addresses is passed as a parameter along with username and password values.
...
Without this parameter, Set-HPiLOSecurityMessage requires you to provide the values of all required parameters.
