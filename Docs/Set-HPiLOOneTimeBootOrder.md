...
## SYNOPSIS
Sets one time boot order.
(C) Copyright 2013, 2014 Hewlett-Packard Development Company, L.P.

## SYNTAX

```
Set-HPiLOOneTimeBootOrder [-OutputType <String>] [-Username <Object>] [-Password <Object>]
 [-Credential <Object>] [-Force] [-Server <Object>] [-Device <Object>] [-BootCustomName <Object>]
 [<CommonParameters>]
```

## DESCRIPTION
The Set-HPiLOOneTimeBootOrder cmdlet sets one time boot order.
Note: The possible set of values for the boot device is different for different iLOs.

iLO 3 version:
This will take one boot device from "Normal", "Floppy", "CDROM", "HDD" ,"USB","RBSU" or "Network"

iLO 4 version or later:
This will take one boot device from "Normal", "Floppy", "CDROM", "HDD", "USB" ,"RBSU" ,"Network" 
"UEFI_SHELL" ,"Intelligent_Provisioning" ,"BootCustom" ,"EMB-MENU" ,"EMB-ACU" ,"EMB-HPSUM" or "EMB-DIAGS"
...
### EXAMPLE 1
```
PS C:\> Set-HPiLOOneTimeBootOrder
...
Device name is not provided for the following iLO Server(s):
1.4.217.131
1.4.217.187
1.4.209.53
Use same device name for these servers? (Y/N) : N
Enter device name for 1.4.217.131: CDROM
Enter device name for 1.4.217.187: CDROM
Enter device name for 1.4.209.53: HDD
```
...
### EXAMPLE 2
```
PS C:\> Set-HPiLOOneTimeBootOrder -Server $Server
...
1.4.217.187
1.4.209.53
Use same device name for these servers? (Y/N) : Y
Please enter device name : CDROM
```

$Server is passed as parameter to Set-HPiLOOneTimeBootOrder.
...
### EXAMPLE 3
```
PS C:\> Set-HPiLOOneTimeBootOrder -Server $Server -Username $Username -Password $Password -Device "CDROM"
```
...
### -Device
Specifies the Device for one time boot order.
The set of acceptable values is different for different iLOs.
iLO 3 version:
This will take one boot device from "Normal", "Floppy", "CDROM", "HDD" ,"USB" ,"RBSU" or "Network"
...
Note:UEFI_SHELL is only valid on systems that support UEFI and BootCustom is only valid on systems that support 
UEFI and are not in Legacy mode.Use GET_PERSISTENT_BOOT to see available values.
if Device's value is BootCustom,
BootCustomName becomes a need input parameter and the format likes Boot0009.
...
### -BootCustomName
if Device's value is BootCustom,BootCustomName becomes a need input parameter and the value format likes Bootxxxx(Boot0009).
...
Without this parameter, Set-HPiLOOneTimeBootOrder requires you to provide the values of all required parameters.
