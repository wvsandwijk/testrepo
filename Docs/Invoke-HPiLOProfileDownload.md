...
## SYNOPSIS
Modifies a Profile Description, downloads a specific blob, and writes the blob to the blob store.
(C) Copyright 2013, 2014 Hewlett-Packard Development Company, L.P.

## SYNTAX

```
Invoke-HPiLOProfileDownload [-OutputType <String>] [-Username <Object>] [-Password <Object>]
 [-Credential <Object>] [-Force] [-Server <Object>] [-DescName <Object>] [-ProfileName <Object>]
 [-Description <Object>] [-BLOBNameSpace <Object>] [-BLOBName <Object>] [-URL <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Invoke-HPiLOProfileDownload cmdlet adds the description to a profile.
It also downloads a specific BLOB to the BLOB store.
Config iLO privilege is required to execute this command.
A blob store is a region of memory that is accessible by both the iLO and the embedded software environment.
Management software can communicate with the embedded environment through the iLO by reading and writing files to the BLOB store.
...
### EXAMPLE 1
```
PS C:\> Invoke-HPiLOProfileDownload -URL $URL
...
1.4.217.187
1.4.209.53
Use same DescName for these servers? (Y/N) : N
Enter DescName for 1.4.217.131 (iloqwc.company.net): Windows_settings12
Enter DescName for 1.4.217.187 (ilo.company.net)   : Windows_settings12
Enter DescName for 1.4.209.53  (ilomxq.company.net): Windows_settings7
```
...
### EXAMPLE 2
```
PS C:\> Invoke-HPiLOProfileDownload -Server $Server
...
$Server is passed as parameter to Invoke-HPiLOProfileDownload.
...
### EXAMPLE 3
```
PS C:\> Invoke-HPiLOProfileDownload -Server $Server -Username $Username -Password $Password -DescName $DescName -BLOBNamespace $BLOBNamespace -BLOBName $BLOBName -URL $URL
```

This command shows that the list of iLO server addresses is passed as a parameter along with usernames, passwords and required parameters to execute the command.
...
### -Description
Specifies a decription of the profile.
This value is optional, and accepts free-form text.
...
Used in conjunction with BLOBName when URL is not used.
...
Only alphanumerics and underscores are accepted; spaces, periods, and slashes are invalid.
HP recommends that you use the same value as DescName.
Used in conjunction with BLOBNameSpace when URL is not used.
...
Without this parameter, Invoke-HPiLOProfileDownload requires you to provide the values of all required parameters.
