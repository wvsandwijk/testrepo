...
## SYNOPSIS
Toggles the power on the host server.
(C) Copyright 2013, 2014 Hewlett-Packard Development Company, L.P.

## SYNTAX

```
Set-HPiLOHostPower [-OutputType <String>] [-Username <Object>] [-Password <Object>] [-Credential <Object>]
 [-Force] [-Server <Object>] [-HostPower <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Set-HPiLOHostPower cmdlet toggles the power on the host server in which the iLO is executing.
This command uses the Advanced Configuration and Power Interface (ACPI) Power Button function to attempt to gracefully shutdown the host operating system (if your OS supports and is configured for graceful shutdown) before it powers off the server.
...
1.4.217.187
1.4.209.53
Use same HostPower value for these servers? (Y/N) : N
Enter HostPower for 1.4.217.131: Yes
Enter HostPower for 1.4.217.187: No
Enter HostPower for 1.4.209.53: Yes
```
...
### EXAMPLE 2
```
PS C:\> Set-HPiLOHostPower -Server $Server -HostPower @("On","On","On")
...
This command takes parameters for HostPower
$Server is passed as parameter to Set-HPiLOHostPower.
...
### EXAMPLE 3
```
PS C:\> Set-HPiLOHostPower -Server $Server -Username $Username -Password $Password -HostPower @("On","Off","On")
```

This command shows that the list of iLO server addresses is passed as a parameter along with usernames, passwords, and required paramter i.e.
HostPower.
...
### -HostPower
Values "No" or "Off" turn host server power off.
A graceful shutdown will be attempted for ACPI-aware operating systems configured to support graceful shutdown.
Values "Yes" or "On" turn host server power on
Allowed values are Yes or On for switching the host power on and No or Off for switching the host power off.
...
Without this parameter, Set-HPiLOHostPower requires you to provide the values of all required parameters.
