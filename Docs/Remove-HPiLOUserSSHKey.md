...
```
Remove-HPiLOUserSSHKey [-OutputType <String>] [-Username <Object>] [-Password <Object>] [-Credential <Object>]
 [-Force] [-Server <Object>] [-UserLoginToEdit <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Remove-HPiLOUserSSHKey cmdlet removes any SSH keys associated with a particular UserLogin.
Admin privilege is required to execute this cmdlet.
A list of servers(with or without port number) and corresponding username/password or credential and UserLoginToEdit values must be provided as parameters.

## EXAMPLES

### EXAMPLE 1
```
PS C:\> Remove-HPiLOUserSSHKey
...
UserLoginToEdit is not provided for the following iLO Server(s):
1.4.217.131
1.4.217.187
1.4.209.53
Delete SSH keys for same UserLogin for these servers? (Y/N) : Y
Please enter UserLogin to remove SSH keys: SusanLee
```
...
### EXAMPLE 2
```
PS C:\> Remove-HPiLOUserSSHKey -Server $Server
...
$Server is passed as parameter to Remove-HPiLOUserSSHKey.
...
### EXAMPLE 3
```
PS C:\> Remove-HPiLOUserSSHKey -Server $Server -Username $Username -Password $Password -UserLoginToEdit $userLoginToEdit
```
...
Without this parameter, Remove-HPiLOUserSSHKey requires you to provide the values of all required parameters.
