...
```
Set-HPiLODefaultLanguage [-OutputType <String>] [-Username <Object>] [-Password <Object>]
 [-Credential <Object>] [-Force] [-Server <Object>] [-LanguageID <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Set-HPiLODefaultLanguage cmdlet sets the default language on iLO.
...
LanguageID is not provided for the following iLO Server(s):
1.4.217.131
1.4.217.187
1.4.209.53
Use same LanguageID for these servers? (Y/N) : Y
Please enter LanguageID : en
```
...
### EXAMPLE 2
```
PS C:\> Set-HPiLODefaultLanguage -Server $Server
...
1.4.209.53
Use same LanguageID for these servers? (Y/N) : Y
Please enter LanguageID : ZH
```

$Server is passed as parameter to Set-HPiLODefaultLanguage.
It can be a string array having iLO IP addresses in string format or it can be an array of PowerShell objects having iLO details including IP address. 
Because the username, password, and languageID are not provided for the iLOs, you are asked to input these values.

### EXAMPLE 3
```
PS C:\> Set-HPiLODefaultLanguage -Server $Server -Username $Username -Password $Password -LanguageID $LanguageID
```
...
### -LanguageID
Specifies the language ID which is the two letter designation for a language.
This parameter must not be blank.
Examples for LanguageID are:
EN (English)
JA (Japanese)
...
Without this parameter, Set-HPiLODefaultLanguage requires you to provide the values of all required parameters.
