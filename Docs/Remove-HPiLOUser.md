...
```
Remove-HPiLOUser [-OutputType <String>] [-Username <Object>] [-Password <Object>] [-Credential <Object>]
 [-Force] [-Server <Object>] [-RemoveUserLogin <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Remove-HPiLOUser cmdlet removes an existing local user account.
The RemoveUserLogin parameter must exist in the current user database.
You must have the Administer User Accounts privilege.
...
1.4.217.187
1.4.209.53
Remove same user from these servers? (Y/N) : Y
Please enter user login to remove : suzylee
```
...
### EXAMPLE 2
```
PS C:\> Remove-HPiLOUser -Server $Server
...
Userlogin to remove is not provided for the following iLO Server(s):
1.4.217.131
1.4.217.187
1.4.209.53
Remove same user from these servers? (Y/N) : Y
...
$Server is passed as parameter to Remove-HPiLOUser.
...
### EXAMPLE 3
```
PS C:\> Remove-HPiLOUser -Server $Server -Username $Username -Password $Password -RemoveUserLogin $RemoveUserLogin
```
...
### -RemoveUserLogin
Specifies login name of the user account to be removed.
...
Without this parameter, Remove-HPiLOUser requires you to provide the values of all required parameters.
