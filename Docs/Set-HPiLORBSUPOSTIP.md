...
```
Set-HPiLORBSUPOSTIP [-OutputType <String>] [-Username <Object>] [-Password <Object>] [-Credential <Object>]
 [-Force] [-Server <Object>] [-RBSUPOSTIP <Object>] [<CommonParameters>]
```

## DESCRIPTION
The Set-HPiLORBSUPOSTIP cmdlet configures the management processor RBSU to display the IP address during POST.
...
RBSU POST IP is not provided for the following iLO Server(s):
1.4.217.131
1.4.217.187
1.4.209.53
Use same RBSU POST IP for these servers? (Y/N) : N
Enter RBSU POST IP value for 1.4.217.131: Y
Enter RBSU POST IP value for 1.4.217.187: Y
Enter RBSU POST IP value for 1.4.209.53: N
```
...
### EXAMPLE 2
```
PS C:\> Set-HPiLORBSUPOSTIP -Server $Server
...
1.4.217.187
1.4.209.53
Use same RBSU POST IP for these servers? (Y/N) : Y
Please enter RBSU POST IP value : Y
```

$Server is passed as parameter to Set-HPiLORBSUPOSTIP.
...
### EXAMPLE 3
```
PS C:\> Set-HPiLORBSUPOSTIP -Server $Server -Username $Username -Password $Password -RBSUPOSTIP $RBSUPOSTIP
```
...
### -RBSUPOSTIP
If RBSUPOSTIP is Yes, Y Enabled or On then the iLO IP address is displayed during POST following the iLO firmware version number. 
If RBSUPOSTIP is No, N, Disable or Off then the iLO IP address would not be displayed during POST following the iLO firmware version number.
...
Without this parameter, Set-HPiLORBSUPOSTIP requires you to provide the values of all required parameters.
